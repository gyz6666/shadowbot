...
- 使用 `python scripts/rpa.py`（Linux 若无 `python` 则用 `python3`）。
...
3. 参数类型处理：
   ...
   - **`type: "file_url"`**：告诉用户需要上传文件；请用户发送文件后，执行 `python scripts/upload_file.py "<文件路径>" --quiet` 获取公网 URL；将 URL 作为该参数的值。文件路径通常在 `media/inbound/` 目录下。
