...
def parse_argv_json():
    """Parse action JSON from argv. Supports three modes:
    1. Direct:  python rpa.py '{"action":"robots"}'
    2. File:    python rpa.py --file tmp.json
    3. Stdin:   echo '{"action":"robots"}' | python rpa.py --stdin

    Mode 2/3 are Windows-safe — PowerShell cannot be trusted to pass quoted JSON.
    """
    if len(sys.argv) < 2:
        return {}
    ...
    # Direct JSON (legacy, works on bash but NOT PowerShell)
